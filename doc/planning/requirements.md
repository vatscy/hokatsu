...
- **JSON エクスポート / インポート（簡易版）**
  ...
  - 用途: 端末故障時のバックアップ、v2→v3 のホスティング移行時のデータ持ち越し
  - マージ機能は v3 で実装する想定。v1 では「全件上書き」のみで割り切る
- **文字列でデータ共有**
  - 全データを deflate-raw 圧縮 + base64url エンコードした短縮文字列（`h1:...` 形式）に変換してクリップボードにコピー
  - コピーボタン押下中は「コピー中…」を表示してボタンを無効化（連打防止）。完了後 2 秒間「コピー完了」を表示してから初期表示に戻る
  - コピーした文字列を別端末のブラウザに貼り付けてインポートできる（全件上書き）
  - インポート完了後は自動で一覧画面へ遷移する
  - 用途: ファイルを介さない端末間の手軽なデータ移行（LINE やメールでテキストとして送付するなど）
...
### v3（手動共有・拡張 / 必要に応じて）
v1 で実装した JSON 入出力を「夫婦間の共有」に耐えられる形に強化する軽量フェーズ。
...
- 文字列でデータ共有（deflate-raw 圧縮 + base64url の短縮文字列でコピー＆ペースト共有。コピーは操作中フィードバック付き。インポート完了後は自動で一覧画面へ遷移）
- v3: Google Drive 自動同期
...
- **フェーズ別ホスティング切り替えの背景**:
  ...
  - v3-v4 は JSON のインポート・エクスポート、Google Drive 連携など、データ取り扱いや認証ロジックが絡む。ソースコードを公開リポジトリに置きたくないため、private リポジトリに切り替える。
...
- **JSON 共有時のマージ戦略**: v3 でインポート時に同一園が既存データにある場合、上書き / スキップ / 別レコードとして追加のいずれにするか。UI でユーザに選ばせる方針で設計。
...
- **v2 → v3 移行時の作業**:
  ...
  - ローカル保存データ（IndexedDB）はドメインが変わると引き継げないため、移行前に v1 の JSON エクスポート機能でバックアップ → 新ドメインでインポートする手順で対応
