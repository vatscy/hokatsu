...
- **共有文字列によるデータ共有（バックアップ・端末間移行を兼ねる）**
  - 全データを deflate-raw 圧縮 + base64url エンコードした短縮文字列（`h1:...` 形式）に変換する
...
- 編集・削除
- 共有文字列によるデータ共有（deflate-raw 圧縮 + base64url の短縮文字列、`h1:` 形式）
...
- IndexedDB 保存
- 共有文字列機能（deflate-raw + base64url、`h1:` 形式）によるクリップボード共有
