...
- **入力項目は紙の記録表と同じ全項目**（「4.1 入力（最終形）」のセクション構成に準拠）
  - 基本情報 / 運営条件 / 施設 / 日常運用 / 費用 / 人物（5段階印象評価含む）/ その他
  - 全項目空欄でも保存可能
  - セクションごとに折りたたみ可能なフォーム
- 園一覧（カード or リスト表示、並び替え: 見学日 / 印象評価平均 / 園名 / 家からの距離）
- ローカル保存（IndexedDB or localStorage）
- スマホ・PC両対応のレスポンシブUI
- **共有文字列によるデータ共有（バックアップ・端末間移行を兼ねる）**
  - 全データを「キー名短縮 + 空値プルーニング + 日本語 enum の数値化 + 日時 base36 化」を施した JSON に変換し、deflate-raw + base64url で短縮文字列（`h1:...` 形式）にエンコードする
  - キー短縮テーブル・enum コードは [src/lib/shareCodec.ts](../../src/lib/shareCodec.ts) を一次情報とする。`Kindergarten` 型を変更したら同ファイルの `KINDERGARTEN_KEY_MAP` も同 PR で追従させる
  - 共有手段は 2 通り。いずれも同じ文字列フォーマットを扱う:
    - **クリップボードコピー / ペースト**: ファイルを介さずに別端末へ貼り付けて取り込み（LINE やメール送付などを想定）
    - **txt ファイルのダウンロード / アップロード**: 共有文字列を `.txt` として書き出し / 読み込み。端末故障時のバックアップやホスティング移行時のデータ持ち越し用途を兼ねる
  - インポートは「全件上書き」のみ（マージは v3 で検討）
  - コピーボタン押下中は「コピー中…」を表示してボタンを無効化（連打防止）。完了後 2 秒間「コピー完了」を表示してから初期表示に戻る
  - インポート完了後は自動で一覧画面へ遷移する

### v2（比較・可視化・オフライン対応 / 1〜2週間後）
...
### v3（手動共有・拡張 / 必要に応じて）
v1 で実装した共有文字列入出力を「夫婦間の共有」に耐えられる形に強化する軽量フェーズ。

- 共有文字列インポートの **マージ対応強化**
  - 相手のデバイス（スマホ・PC）に手動で共有（クリップボード / txt ファイルを LINE / メール / AirDrop などで送付）
...
### v4（自動同期 / 余裕があれば）
真の意味でのリアルタイム同期。v3 の手動運用に限界を感じた場合に着手。

- Google Drive 連携によるクラウド同期
  - データ本体は共有文字列（または JSON）として Drive の app data folder 上に保存
...
| セクション | 主な項目 |
|---|---|
| 基本情報 | 園名 / 見学日 / 時間 / 分類 / 家から距離・時間 / 駅から距離・時間 / 会社から距離・時間 / メモ |
| 運営条件 | 休園期間 / 祝日保育 / 保育時間 / 延長保育 / クラス定員 / 現在の空き人数 / メモ |
| 施設 | 園庭 / おでかけ頻度 / 近隣公園 / プール / 駐輪場 / メモ |
| 日常運用 | 服装 / 給食費 / 給食の自園調理 / お弁当 / 慣らし保育 / 連絡帳 / お迎えの判断（発熱基準）/ 保護者参加行事 / 保護者会役員 / 朝やること / 園内の習い事 / 手作りが必要なもの / 布団 / SNS / メモ |
| 費用 | 月々の諸費用 / サブスク可能なもの / おむつの処分 / メモ |
| 人物 | 案内者 / 印象5段階評価（5項目） / メモ |
| その他 | 自由メモ / 1日のスケジュール |
...
| 分類 | 単一選択 | 公立 / 認可 / 認可外 |
| 家から | 数値（km）＋ 数値（分） | |
| 駅から | 数値（km）＋ 数値（分） | 最寄り駅からの距離・時間 |
| 会社から | 数値（km）＋ 数値（分） | |
| メモ | テキスト（複数行） | セクション内の補足メモ |
...
| 園内の習い事 | テキスト（複数行） | |
| 手作りが必要なもの | テキスト（複数行） | |
| 布団 | テキスト | 持参・レンタル・サイズ等を自由記述 |
| SNS | テキスト | 園が運用する SNS（アカウント・URL 等）を自由記述 |
| メモ | テキスト（複数行） | セクション内の補足メモ |
...
- 園を一覧表示（並び替え：見学日 / 印象評価平均 / 園名 / 家からの距離）
  - 家からの距離は「距離（km）」「時間（分）」のいずれかで並び替え可能。未入力の園は末尾扱い。
- 比較ビュー
  - レーダーチャート（複数園を重ねて表示、v2）
  - 横並び比較表（v2）

### 4.3 データ操作

- 編集・削除
- 共有文字列によるデータ共有（キー短縮 + 空値プルーニング + enum 数値化 + 日時 base36 化を施した JSON を deflate-raw + base64url、`h1:` 形式）
  - クリップボードへのコピー＆ペーストでの共有
  - `.txt` ファイルとしてのダウンロード／アップロードでの共有・バックアップ
  - コピーは操作中フィードバック付き。インポート完了後は自動で一覧画面へ遷移
  - インポートは「全件上書き」のみ（マージは v3 で検討）
- v4: Google Drive 自動同期
...
| オフライン | v2 から見学中のオフライン入力に対応（PWA）。v1 はオンライン前提（一度ロードすれば SPA として動くが、Service Worker による保証はなし）|
| データ保存 | v1-v3: ブラウザローカル（IndexedDB 推奨）／ v4: Google Drive |
| 認証 | v1-v3: なし／ v4: Google OAuth（クライアントサイドのみ）|
...
|---|---|---|---|
| v1-v2 | **public** | **GitHub Pages** | `https://<username>.github.io/<repo>/` |
| v3-v4 | **private** | **別の静的ホスティング（Cloudflare Pages / Netlify / Vercel など）** | 各サービスのドメイン or 独自ドメイン |

- **フェーズ別ホスティング切り替えの背景**:
  - v1-v2 はテキスト・評価のみの素朴な構成で、ソースコードが公開されても支障がない。GitHub Pages を無料で使える public リポジトリで運用。
  - v3-v4 は共有文字列のマージインポート、Google Drive 連携など、データ取り扱いや認証ロジックが絡む。ソースコードを公開リポジトリに置きたくないため、private リポジトリに切り替える。
...
- **その他の派生設計方針**:
  - v4 の Google Drive 同期はサーバ経由ではなく **ブラウザから直接 Google OAuth + Drive API を叩く** 方式（API キー・クライアント ID は配信物として公開される前提でスコープを Drive の app data folder 等に限定）。
  - ルーティングは HashRouter（`#/...`）を採用する。GitHub Pages はリロード時のサーバサイドルーティングを持たないため。v3 以降の移行先でも同方針を維持し、URL の互換性を保つ。
  - フェーズ移行時は同じビルド成果物が動くよう、ホスティング非依存（ベースパス設定のみ環境変数で切り替え）の構成を v1 から徹底する。

## 6. データモデル概要

[src/types/kindergarten.ts](../../src/types/kindergarten.ts) を正とする。本節はそのスナップショット（変更時は実装側を更新後、こちらも追従）。型を増減した場合は共有文字列のキー短縮テーブル [src/lib/shareCodec.ts](../../src/lib/shareCodec.ts) も同 PR で追従させること。
...
interface Kindergarten {
  ...
  generalMemo?: string;       // 基本情報セクションのメモ
  ...
  operationMemo?: string;     // 運営条件セクションのメモ
  ...
  pool?: boolean | null;
  bikeParking?: boolean | null; // 駐輪場 有無
  facilityMemo?: string;        // 施設セクションのメモ

  // --- 日常運用 ---
  clothing?: ClothingInfo;
  lunchFee?: LunchFee;
  inHouseLunch?: boolean | null; // 給食の自園調理 有無
  ...
  lessons?: string;          // 園内の習い事（旧「園内活動」から移設）
  handmadeItems?: string;    // 手作りが必要なもの（旧「園内活動」から移設）
  bedding?: string;          // 布団（持参・レンタル・サイズ等を自由記述）
  sns?: string;              // 園が運用する SNS（アカウント・URL 等を自由記述）
  dailyOperationMemo?: string; // 日常運用セクションのメモ
  ...
  costMemo?: string;          // 費用セクションのメモ

  // --- 人物 ---
  guide?: GuidePerson;
  impressions?: Impressions;
  peopleMemo?: string;        // 人物セクションのメモ
...
- **共有文字列インポート時のマージ戦略**: v3 でインポート時に同一園が既存データにある場合、上書き / スキップ / 別レコードとして追加のいずれにするか。UI でユーザに選ばせる方針で設計。
- **Google Drive 同期の競合解決**: 夫婦が同じ園を同時編集した場合の競合をどう扱うか。v4 設計時に検討。
...
- **v2 → v3 移行時の作業**:
  ...
  - 移行先ホスティングの選定は **v2 完了タイミングで実施**（候補: Cloudflare Pages / Netlify / Vercel）。判断軸: 無料枠 / カスタムドメイン / ビルド時間 / PWA 対応 / プライベートリポジトリでの利用条件
  - 既存ユーザ（夫婦）への URL 変更告知とブックマーク更新
  - 旧 GitHub Pages 側の停止または「移行先へリダイレクト」ページへの差し替え
  - ローカル保存データ（IndexedDB）はドメインが変わると引き継げないため、移行前に v1 の共有文字列エクスポート（txt ファイル）でバックアップ → 新ドメインでインポートする手順で対応
- **Google Drive 同期の現実性（v4）**: バックエンドなしで OAuth + Drive API を叩く構成は実装可能だが、トークン管理やリフレッシュをクライアント側で扱う必要があり、v1-v3 の設計に比べて複雑度が跳ねる。v3 の手動共有運用で支障がなければ v4 は着手しない判断もあり得る。
...
- IndexedDB 保存
- 共有文字列機能（キー短縮 + 空値プルーニング + enum 数値化 + 日時 base36 化を施した JSON を deflate-raw + base64url、`h1:` 形式）によるクリップボード共有
- GitHub Pages へ自動デプロイ（HashRouter）

#### v1 の残対応（本変更で着手）

- JSON エクスポート / インポート機能を撤去（共有文字列に一本化）
- 共有文字列の **txt ファイルダウンロード / アップロード** 対応を追加
