...
| 1日のスケジュール | テキスト（時間帯ごとの自由記述） | 7〜19時 |

### 4.2 一覧・比較

- 園を一覧表示（並び替え：見学日 / 印象評価平均 / 園名 / 家からの距離）
  - 家からの距離は「距離（km）」「時間（分）」のいずれかで並び替え可能。未入力の園は末尾扱い。
  - 並び替え（項目・昇降）の選択はブラウザに保存し、次回以降の表示に自動適用する。デフォルトは「見学日 × 降順」。
- 園カードはアコーディオン形式。
  - ヘッダ常時表示部に概要（園名・見学日・分類・家から所要分・印象評価平均）と編集・削除のアイコンボタンを並べる。
  - ヘッダを開くと全セクションの入力済み内容を読み取り専用で閲覧できる（値が空のセクション・項目はまとめて省略）。
  - 編集／削除アイコン押下は開閉と切り離す（アコーディオンは開閉しない）。
- 比較ビュー
...
- 一覧の並び替え：見学日 / 印象評価平均 / 園名 / 家からの距離（km・分）。設定は localStorage に保存して次回起動時に自動復元。
- 園カードはアコーディオン形式で全項目の読み取り専用閲覧に対応（編集・削除はアイコンボタン）。
- IndexedDB 保存
- 共有文字列機能（クリップボード共有・txt ファイルダウンロード / アップロード）
